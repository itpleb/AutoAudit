...
--BtAudit makes several changes to your tables.				--
--Adding BtAudit to your tables will impact performance.		--
--Set the variables below to your requirements before executing.--
--**************************************************************--

set nocount on
--USE BtAudit -- edit for your database
...
--*******************************************************
Set @AuditSchema		= 'BtAudit'			--This is the schema to use for the BtAudit objects. Edit for your database

Set @ViewSchema			= '<TableSchema>'	--This is the schema to use for the BtAudit base table views. Edit for your database.
											--<TableSchema> = the _RowHistory and _Deleted views have the same schema as the base table.

Set @Version			= '3.30a-modified'			--leave this unless you are making changes to this script

Set @OptimizeForAudit	= 0					--@OptimizeForAudit = 0 creates an index to speed up views
											--(and slows down BtAudit), @OptimizeForAudit = 1 keeps BtAudit 10% faster 
											--but the reporting views are slower.

Set @RebuildTriggersAfterInstall = 1		--@RebuildTriggersAfterInstall = 1 launches pBtAuditRebuildAll after
											--this script has completed and BtAudit has been updated
											--@RebuildTriggersAfterInstall = 0 runs this script to update BtAudit but does NOT 
											--execute pBtAuditRebuildAll. This may cause problems with your existing BtAudit
											--triggers views and UDFs.

Set @DetailedMigrationCheck	= 0				--This is only applicable if you are upgrading your BtAudit environment
...
--*** IF YOU ARE UPGRADING FROM BtAudit 2.X set the column names to Created, CreatedBy, Modified, ModifiedBy and RowVersion 
--    otherwise the upgrade process will add new columns. 
--    YOU CAN CHANGE THE NAMES LATER IN THE BASE TABLES AND THE BtAuditSETTINGS TABLE.
Set @CreatedColumnName = 'BtAudit_CreatedDate'
Set @CreatedByColumnName = 'BtAudit_CreatedBy'
Set @ModifiedColumnName = 'BtAudit_ModifiedDate'
Set @ModifiedByColumnName = 'BtAudit_ModifiedBy'
Set @RowVersionColumnName = 'BtAudit_RowVersion'
...
Set @ViewPrefix = 'BtAudit_v'								--User configurable - sets the PREFIX for _RowHistory, _Deleted views
Set @UDFPrefix = 'BtAudit_p'								--User configurable - sets the PREFIX for _RowHistory, _TableRecovery functions
...
Stored Procedures:
  - <AuditSchema>.pBtAudit
  - <AuditSchema>.pAutoAutitDrop 
  - <AuditSchema>.pBtAuditAll
  - <AuditSchema>.pBtAuditDropAll
  - <AuditSchema>.pBtAuditArchive (new for 3.00)
  - <AuditSchema>.pBtAuditRebuild (new for 3.00)
  - <AuditSchema>.pBtAuditRebuildAll (new for 3.00)
...
Database DDL Trigger:
  - BtSchemaAuditDDLTrigger DDL Trigger 
...
Important: The 2.00h Audit table is being replaced by a AuditHeader and
           ...
           If you are currently using BtAudit version 2.00h, running this
           ...
           tables.
           Also, all of your existing BtAudit base table triggers will be
...
Limitations:
  ...
  Adding BtAudit triggers to a table will impact performance, 
  potentially doubling or tripling the normal DML execution times. 
  The width of the table increases the impact of the BtAudit 
  triggers during updates. 


BtAudit database object description
--------------------------------------------------------------------
--------------------------------------------------------------------
...
This table contains all the rows that originated in the AuditHeader
    ...
    <AuditSchema>.pBtAuditArchive procedure.
...
captured by the BtSchemaAuditDDLTrigger database trigger.
...
Important:
	After running this script to install BtAudit, please review
...
Here's a list of entries in the AuditSettings table:
	...
	AdditionalInfo: System setting added by BtAudit installation 
	...
	AdditionalInfo: User configurable - Schema BtAudit uses for 
					...
					is used, the schema of the BtAudit views for
	...
	SettingName:	BtSchemaAuditDDLTrigger Enabled Flag
	...
	AdditionalInfo: User configurable - Immediate change. No action 
					...
					pBtAuditArchive stored procedure is executed.
	...
	AdditionalInfo: User configurable - Must execute pBtAuditRebuild(All) 
					or pBtAudit(All) to apply change. Determines 
	...
	SettingName:	Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag
	SettingValue:	1 (default)
	AdditionalInfo: System setting added by BtAudit installation 
					SQL script.  Do not change manually in the table.
	...
	AdditionalInfo: User configurable
                    0 = Debug information (The trigger name and nest level) in returned
                        by the BtAudit Insert, Update and Delete triggers.
	...
	SettingValue:	BtAudit_Created (default)
	AdditionalInfo: User Configurable - Sets the column name of the column that is added
					to the base tables to save the "record creation date" entry by BtAudit
					when "@BaseTableDDL = 1" is set in the execution of pBtAudit.

	SettingName:	CreatedByColumnName
	SettingValue:	BtAudit_CreatedBy (default)
	AdditionalInfo: User Configurable - Sets the column name of the column that is added
					to the base tables to save the "record created by" entry by BtAudit
					when "@BaseTableDDL = 1" is set in the execution of pBtAudit.

	SettingName:	ModifiedColumnName
	SettingValue:	BtAudit_Modified (default)
	AdditionalInfo: User Configurable - Sets the column name of the column that is added
					to the base tables to save the "record last modifocation date" entry
					by BtAudit when "@BaseTableDDL = 1" is set in the execution of pBtAudit.

	SettingName:	ModifiedByColumnName
	SettingValue:	BtAudit_ModifiedBy (default)
	AdditionalInfo: User Configurable - Sets the column name of the column that is added
					to the base tables to save the "record last modified by" entry by BtAudit
					when "@BaseTableDDL = 1" is set in the execution of pBtAudit.

	SettingName:	RowVersionColumnName
	SettingValue:	BtAudit_RowVersion (default)
	AdditionalInfo: User Configurable - Sets the column name of the column that is added
					to the base tables to save the "record verion number" entry by BtAudit
					when "@BaseTableDDL = 1" is set in the execution of pBtAudit.

	SettingName:	ViewPrefix
	SettingValue:	v (default)
	AdditionalInfo:	User configurable (default = "v") - Must execute pBtAuditRebuild(All) or 
					pBtAudit(All) to apply change. Sets the prefix to use for the _RowHistory 
					and _Deleted views.

	SettingName:	DateStyle
	SettingValue:	121 (default)
	AdditionalInfo:	System setting added by BtAudit installation SQL script. Do not change 
					manually in the table. You can re-run the BtAudit installation script to 
					change this setting.

	SettingName:	UDFPrefix
	SettingValue:	'' (default)
	AdditionalInfo:	User configurable (default = "") - Must execute pBtAuditRebuild(All) or 
					pBtAudit(All) to apply change. Sets the prefix to use for the _RowHistory 
	...
	AdditionalInfo:	User configurable (default = "_RowHistory") - Must execute 
					pBtAuditRebuild(All)  or pBtAudit(All) to apply change. Sets the suffix 
					to use for the _RowHistory views.

	SettingName:	DeletedViewSuffix
	SettingValue:	_Deleted (default)
	AdditionalInfo:	User configurable (default = "_Deleted") - Must execute pBtAuditRebuild(All) 
					or pBtAudit(All) to apply change. Sets the suffix to use for the _Deleted 
					views.

	SettingName:	RowHistoryFunctionSuffix
	SettingValue:	_RowHistory (default)
	AdditionalInfo:	User configurable (default = "_RowHistory") - Must execute 
					pBtAuditRebuild(All) or pBtAudit(All) to apply change. Sets the suffix to 
...
This table contains one row for each of the base tables that was 
    setup to use the BtAudit system.  The data in this table can
    be useful for a DBA who wants to review which base tables are
    setup for Auto 
Changes to the BtAudit configuration for each table can be done in 
    this table.
The pBtAuditRebuild stored procedure is dependant on the data in 
    this table to work.
...
This table contains a user-defined list or base tables to exclude 
    when the pBtAuditAll stored procedure is executed.
...
This view does a Union All of the data from the vAudit and 
    ...
    contained in the BtAudit (Active and Archive) tables.
...
This view does a Union All of the data from the AuditHeader and 
    ...
    header data contained in the BtAudit (Active and Archive) 
    tables.
...
This view does a Union All of the data from the AuditdDetail and 
    ...
    detail data contained in the BtAudit (Active and Archive) 
    tables.

--------------------------------------------------------------------
--------------------------------------------------------------------
* <AuditSchema>.pBtAudit Procedure

applies BtAudit to a single table
...
---
pBtAudit will make the following changes:
...
@StrictUserContext determines how user context columns are set
    ...
    Turning @StrictUserContext off is useful for two use cases: 
       1 - applications that manage their own user security and log into 
           ...
           can pass the user name to BtAudit by inserting into the base 
...
Regardless of the @LogInsert and @LogUpdate setting, the Created, CreatedBy,  
   ...
   set if BtAudit was added to the table with option @BaseTableDDL = 1. 

---

To change the options for a table, simply re-exec the pBtAudit proc  
    with the required options to re-generate the triggers for the table.
    Alternatively, you can update the BtAudit settings changes on a
    table by table basis in the <AuditSchema>.AuditBaseTables table
    and then execute <AuditSchema>.pBtAuditRebuild or
    <AuditSchema>.pBtAuditRebuildAll.

--------------------------------------------------------------------
--------------------------------------------------------------------
* <AuditSchema>.pBtAuditAll Procedure

Executes pBtAudit for every basetable except for the tables listed
...
--------------------------------------------------------------------
--------------------------------------------------------------------
* <AuditSchema>.pBtAuditDrop Procedure

Removes columns, triggers, views, and function 
created by pBtAudit for a single table. 
...
parameters: 
  ...
  @DropBaseTableTriggers BIT (Default = 1)
    0 = keeps the base table BtAudit Triggers
    1 = (default) drops the base table BtAudit Triggers
  @DropBaseTableViews BIT (Default = 1)
    0 = keeps the base table BtAudit views and function
    1 = (default) drops the base table BtAudit views and function

It does not remove the audit tables or SchemaAudit 
trigger or table created when this script is executed 
in a database.

If your intention is to keep the BtAudit triggers but drop the
DDL columns, you will also have to use <AuditSchema>.pBtAuditRebuild

--------------------------------------------------------------------
--------------------------------------------------------------------
* <AuditSchema>.pBtAuditDropAll Procedure

Drops selected components of Auto
Optionally executes pBtAuditDrop for every basetable using the 
   default options.

parameters: 
  @DropAuditTables BIT (Default = 0)
    0 = (default) keeps the BtAudit tables
    1 = drops all the BtAudit tables
  @DropAuditViews BIT (Default = 0)
    0 = (default) keeps the BtAudit views
    1 = drops all the BtAudit views
  @DropAuditSPs BIT (Default = 0)
    0 = (default) keeps the BtAudit SP's
    1 = drops all the BtAudit SP's
  @DropAuditDDLTriggers BIT (Default = 0)
    0 = (default) keeps the BtAudit DDL Trigger
    1 = drops the BtAudit DDL Trigger
  ...
  @DropBaseTableTriggers BIT (Default = 0)
    0 = (default) keeps the base table BtAudit Triggers
    1 = drops the base table BtAudit Triggers 
         from each base table
  @DropBaseTableViews BIT (Default = 0)
    0 = (default) keeps the base table BtAudit views and function
    1 = drops the base table BtAudit views and function
...
When @DropAuditTables, @DropAuditViews or @DropAuditSPs
   are flaged as 1, pBtAuditDropAll removes BtAudit components
...
--------------------------------------------------------------------
--------------------------------------------------------------------
* <AuditSchema>.pBtAuditRebuild Procedure (new for 3.00)
...
This AuditBaseTables table and pBtAuditRebuild sp can be very 
useful if columns have been added/modified in a base table. If 
you need to make BtAudit settings changes to one or more 
...
ColumnNames value for that base table before rebuilding the BtAudit
...
execute the pBtAuditRebuild or pBtAuditRebuildAll procedure.
...
Important:
If "Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag"
    is set to 1 in the <AuditSchema>.AuditSettings table, when the 
    BtSchemaAuditDDLTrigger database DDL trigger fires due to a base
    table schema change, BtSchemaAuditDDLTrigger makes an automagic
    call to pBtAuditRebuild after a ALTER_TABLE event.


--------------------------------------------------------------------
--------------------------------------------------------------------
* <AuditSchema>.pBtAuditRebuildAll Procedure (new for 3.00)

Executes pBtAuditRebuild for every basetable that has  an entry 
...
--------------------------------------------------------------------
--------------------------------------------------------------------
* <AuditSchema>.pBtAuditSetTriggerState Procedure (new for 3.02)

Enables or disables BtAudit triggers at the SQL Server level.  

This is different from the EnabledFlag entry in the AuditBaseTables
tables. The pBtAuditSetTriggerState SP enables or disables the 
...
BtAudit exit before logging the Audit event.
...
--------------------------------------------------------------------
--------------------------------------------------------------------
* <AuditSchema>.pBtAuditSetTriggerStateAll Procedure (new for 3.02)

Executes pBtAuditSetTriggerState for every basetable that has  an entry 
  in the <AuditSchema>.AuditBaseTables table.
...
version 1.08 - June 25, 2008
  ...
  defaults and columns dropped in BtAuditDrop proc

-----------------------------------
version 1.09 - Oct 15, 2008
  fixed @tablename bug in BtAuditDrop
  ...
  BtSchemaAuditDDLTrigger also fires pBtAudit for Alter_Table events for tables with BtAudit
...
version 1.10e - Mar 20, 2010
  ...
  added drop of audit tables and ddl trigger to pBtAuditDropAll
...
-----------------------------------
version 2.00g August, 2010
  fixed drop of SchemaTable in pBtAuditDropAll (reported by Calvin Jones)
  ...
  removed SchemaAudit from pBtAuditAll and pBtAuditDropAll (Reproted by jeffcj)
...
version 3.00 January, 2012 (coded by John Sigouin)
  ...
  6.Added code to drop CreatedBy_df and ModifiedBy_df defaults in pBtAuditDrop
    Added code to drop CreatedBy and ModifiedBy audit columns in pBtAuditDrop
 ...
 10.Added parameter to this script to allow user to specify a schema for the 
    BtAudit tables, SPs, views etc (default is [Audit])
		- the _Deleted views and the _RowHistory objects can also be created 
          in the specified schema (which could be the same as the BtAudit schema),
 ...
 11.Created a new Audit table called [AuditSettings] to store BtAudit 
 ...
 19.Updated the pBtAuditDrop SP to remove the new objects.
 20.Created one new index for performance on AuditHeader.PrimaryKey 
 21.The pBtAudit SP parameter behaviour has been modified to accomodate the 
 ...
 22.Added parameters to pBtAuditAll SP to match the ones in pBtAudit except 
    the table schema and name.
 23.Changed the call to pBtAudit from pBtAuditAll to forward the specified 
    parameters instead of always using defaults.
 24.Added parameters to pBtAuditDropAll SP to pick exactly which BtAudit 
 ...
 25.Changed the call to pBtAuditDrop from pBtAuditDropAll to forward the specified 
    parameters instead of always using defaults.
 26.Added print statements to sp's to inform the administrator exactly what is happening.
 27.Added Quotename(...) delimiters all over. BtAudit will work even if you have a table named 
    [ $chem@].[This is my test crazy table name! ] and a column named [$ Amount].  :-)
    DONE! QuoteName() (suggested by Rob Farley)

 28.The base table schema does not have to be the same as the schema used for the BtAudit tables.
 ...
 31.Created a new table called AuditBaseTables.  
		-Whenever a table is setup for BtAudit, a record with the specific BtAudit 
         for this base table is created in the BtAuditBaseTables table.  Subsequently, 
         BtAudit functionality can be enabled/disabled on a table by table basis by 
		...
		-If the auditing settings (StrictUserContext,LogSQL,BaseTableDDL,LogInsert) needs 
         ...
         have been changed, execute the pBtAuditRebuild SP to implement the new settings.
 ...
 32.Created a new SP called pBtAuditRebuild to re-create the BtAudit trigger on a table 
    if the table's BtAudit settings have been changed in the AuditBaseTables table or if 
    columns have been added/modified in a base table.
 32.Created a new SP called pBtAuditRebuildAll to execute the pBtAuditRebuild sp for 
    each of the tables that exist in the AuditBaseTables table.
 23.Changed [BtSchemaAuditDDLTrigger] trigger to call pBtAuditRebuild after a ALTER_TABLE event.
 34.Forced exclusion of AuditHeader, AuditDetail, SchemaAudit tables in pBtAuditAll SP.
 35.Created a new table called AuditAllExclusions.  All tables listed in this table will be 
    excluded from the pBtAuditAll and pBtAuditRebuildAll SP's.  You can use this to 
    customize your own exclusions.

  Exclude MS tables and Audit schema table from BtAuditAll (suggested by Calvin Jones)
  DONE! exclusion of AuditHeader, AuditDetail, SchemaAudit tables and added a table 
        to specifify any other exclusions.

 36.Created tables to store BtAudit archive data. The tables are called AuditHeaderArchive
 ...
 41.Created a Stored Procedure <AuditSchema>.pBtAuditArchive to move data from the active
 ...
 44.Added Audit Settings to flag if the _RowHistory view, _RowHistory function and _Deleted
    view are created when BtAudit triggers are added to a table.
 45.Added a seperate variable to identify the schema to use for the BtAudit base table views.
 ...
 48.An optional index can be created to improve the performance of the _delete and _RowHistory 
    ...
    BtAudit loging uses about 20% more I/O and is 10% slower. The script parameter 
    @OptimizeForAudit = 0 creates the index to speed up views (and slow down BtAudit) and 
    @OptimizeForAudit = 1 keeps BtAudit faster but the views and archiving of old Audit 
 ...
 49.Restructured the BtAudit Update trigger to correctly record the RowVersion when a query 
...
version 3.01 January, 2012 (coded by John Sigouin)
   ...
   DONE! Add MS_Description extended property to columns added by BtAudit (suggested by Calvin Jones)
   ...
   7. Added the following entries to the AuditAllExclusions tables. 
			...
			- AuditBaseTables
			- AuditSettings
      Setting up BtAudit on these tables is OK but they are excluded by default. If you 
      want BtAudit to be setup on these tables, simply remove the names from the 
      AuditAllExclusions table or use the pBtAudit stored procedure to set them up individually.
   ...
   9. Added a call to pBtAuditDrop in the Database DDL trigger when a table is dropped to
      automatically drop the BtAudit views that were related to that table.

  10. Added code to delete AuditBaseTables record from pBtAuditDrop when a table is dropped.
...
version 3.02 February, 2012 (coded by John Sigouin)
   1. Created stored procedure pBtAuditSetTriggerState that enables/disabled the 
      insert, update and/or delete BtAudit triggers for the specified base table.
      ...
      **********************************************************************************
      WARNING: BE AWARE THAT IF YOU DISABLE BtAudit TRIGGERS YOU WILL NOT HAVE ANY
               ...
               PROTECT YOUR DATA. MAKE SURE YOU RE-ENABLE THE BtAudit TRIGGERS ONCE 
...
version 3.20 January, 2013 (coded by John Sigouin)
   1. Add the @WithLogFlag flag to this script
		...
		- this is added because some DBA's may not have rights to write to the Windows log
		- 0 = exclude "with log", 1 = include "with log"
   2. Add capability for BtAudit to handle tables with multi-column PK's. BtAudit is now capable of 
   ...
   3. Add the @ColumnNames optional parameter to pBtAudit to specify a sub-set of the base tables 
      columns to include in the AuditDetail loging of changes.
        ...
        - The setting for this parameter is written to the AuditBaseTables table as used when BtAudit
...
version 3.20d April, 2013 (coded by John Sigouin)
	1. Added SET ANSI_PADDING ON statement to [BtSchemaAuditDDLTrigger] trigger to resolve bug

-----------------------------------
version 3.20e October, 2013 (coded by John Sigouin)
    1. Set the Trigger order to run first for the BtAudit insert triggers. This change is implemented 
    to process BtAudit insert triggers correctly where there is another insert trigger in the table.  
...
version 3.20f November, 2013 (coded by John Sigouin)
	...
	in the pBtAuditAll sp. Suggested by patrikwiik
...
version 3.20h November, 2013 (coded by John Sigouin)
	...
	during an update operation with the following logic
		...
		- if NULL values are updated to those columns with the user query, then BtAudit uses the 
	...
	4. Removed the @AuditSchema variable declaration and initialization from the BtSchemaAuditDDLTrigger 
	   trigger because it was not used.
	5. Added an option in pBtAuditAll to exclude all tables from specific schemas as required. For 
	   example, if you have tables in schema [MySchema] and you want them to be excluded from 
	   BtAudit trigger creation when you run pBtAuditAll, you can add this exclusion row in the 
	   AuditAllExclusions SchemaName = MySchema, TableName = <All>.
	6. Added RowVersion to the list of column datatypes that are not supported by BtAudit. 
	7. Added filter on primary key column datatypes that are not allowed: 'binary', 'varbinary', 
	   'timestamp', 'rowversion'
	8. Added SQL_Variant datatype columns back in as allowed datatypes for BtAudit columns
	
	
-----------------------------------
version 3.30 December, 2013 (coded by John Sigouin)
	1. Added a new powerful feature to BtAudit, the creation of the _TableRecovery table-valued UDF. 
	...
	3. Added a variable to configure the date style BtAudit uses for entries in the AuditHeader and 
	   AuditDetail tables
	4. Added User option to set the prefix on BtAudit views and UDF (suggested by rosasek)
	5. Added User option to set the suffix on BtAudit views and UDF (why not go all the way! :-) )
	6. Changed the defaults for pBtAuditDropAll to 0 instead of 1 and added a confirmation input 
	   parameter as failsafe measures to prevent accidental removal of BtAudit.
...
-- Create BtAudit schema, tables, indexes
...
If (select count(*) from sys.objects where [name] = 'AuditSettings' and schema_name(schema_id) <> @AuditSchema) > 0
	Begin
		Print 'An existing installation of BtAudit with a different schema has been found in the database.'
		Print 'You must remove the existing BtAudit before installing it to a different schema.'
...
Print getdate()
Print 'Installing BtAudit in database: ' + db_name(db_id())
...
If Exists(select * from sys.triggers where name = 'BtSchemaAuditDDLTrigger')
   DROP TRIGGER BtSchemaAuditDDLTrigger ON Database
...
Print 'BtAudit schema has been set to: ' + quotename(@AuditSchema)

Print 'Creating BtAudit tables'
...
VALUES	(''AuditSchema'',''' + @AuditSchema + ''',''System setting added by BtAudit installation SQL script.  Do not change manually in the table.'');'
		EXEC (@SchemaSQL)
...
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''Schema for _RowHistory and _Deleted objects'',''' + @ViewSchema + ''',''User configurable - Schema BtAudit uses for _RowHistory, _TableRecovery and _Deleted objects. Valid entries can be an existing schema or <TableSchema>. The default is <TableSchema>.'');'
		EXEC (@SchemaSQL)
--Version
SELECT	@SchemaSQL = 'DELETE FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''Version''' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''Version'',''' + @Version + ''',''System setting added by BtAudit installation SQL script. Do not change manually in the table.'');'
		EXEC (@SchemaSQL)
--DateStyle
SELECT	@SchemaSQL = 'DELETE FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''DateStyle''' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''DateStyle'',''' + @DateStyle + ''',''System setting added by BtAudit installation SQL script. Do not change manually in the table. You can re-run the BtAudit installation script to change this setting.'');'
		EXEC (@SchemaSQL)
...
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''CreatedColumnName'',''' + @CreatedColumnName + ''',''System setting added by BtAudit installation SQL script. Do not change manually in the table.'');'
		EXEC (@SchemaSQL)
--CreatedByColumnName
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''CreatedByColumnName'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''CreatedByColumnName'',''' + @CreatedByColumnName + ''',''System setting added by BtAudit installation SQL script. Do not change manually in the table.'');'
		EXEC (@SchemaSQL)
--ModifiedColumnName
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''ModifiedColumnName'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''ModifiedColumnName'',''' + @ModifiedColumnName + ''',''System setting added by BtAudit installation SQL script. Do not change manually in the table.'');'
		EXEC (@SchemaSQL)
--ModifiedByColumnName
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''ModifiedByColumnName'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''ModifiedByColumnName'',''' + @ModifiedByColumnName + ''',''System setting added by BtAudit installation SQL script. Do not change manually in the table.'');'
		EXEC (@SchemaSQL)
--RowVersionColumnName
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''RowVersionColumnName'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''RowVersionColumnName'',''' + @RowVersionColumnName + ''',''System setting added by BtAudit installation SQL script. Do not change manually in the table.'');'
		EXEC (@SchemaSQL)
...
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''ViewPrefix'',''' + @ViewPrefix + ''',''User configurable (default = "v") - Must execute pBtAuditRebuild(All) or pBtAudit(All) to apply change. Sets the prefix to use for the _RowHistory and _Deleted views.'');'
		EXEC (@SchemaSQL)
--UDFPrefix
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''UDFPrefix'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''UDFPrefix'',''' + @UDFPrefix + ''',''User configurable (default = "") - Must execute pBtAuditRebuild(All) or pBtAudit(All) to apply change. Sets the prefix to use for the _RowHistory and _TableRecovery views.'');'
		EXEC (@SchemaSQL)
--RowHistoryViewSuffix
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''RowHistoryViewSuffix'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''RowHistoryViewSuffix'',''' + @RowHistoryViewSuffix + ''',''User configurable (default = "_RowHistory") - Must execute pBtAuditRebuild(All) or pBtAudit(All) to apply change. Sets the suffix to use for the _RowHistory views.'');'
		EXEC (@SchemaSQL)
--DeletedViewSuffix
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''DeletedViewSuffix'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''DeletedViewSuffix'',''' + @DeletedViewSuffix + ''',''User configurable (default = "_Deleted") - Must execute pBtAuditRebuild(All) or pBtAudit(All) to apply change. Sets the suffix to use for the _Deleted views.'');'
		EXEC (@SchemaSQL)
--RowHistoryFunctionSuffix
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''RowHistoryFunctionSuffix'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''RowHistoryFunctionSuffix'',''' + @RowHistoryFunctionSuffix + ''',''User configurable (default = "_RowHistory") - Must execute pBtAuditRebuild(All) or pBtAudit(All) to apply change. Sets the suffix to use for the _RowHistory functions.'');'
		EXEC (@SchemaSQL)
--TableRecoveryFunctionSuffix
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''TableRecoveryFunctionSuffix'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''TableRecoveryFunctionSuffix'',''' + @TableRecoveryFunctionSuffix + ''',''User configurable (default = "_TableRecovery") - Must execute pBtAuditRebuild(All) or pBtAudit(All) to apply change. Sets the suffix to use for the _TableRecovery functions.'');'
		EXEC (@SchemaSQL)

--BtSchemaAuditDDLTrigger Enabled Flag
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''BtSchemaAuditDDLTrigger Enabled Flag'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''BtSchemaAuditDDLTrigger Enabled Flag'',''1'',''User configurable - Immediate change. No action required. 0 = DDL trigger disabled, 1 = DDL trigger enabled.'');'
		EXEC (@SchemaSQL)
--Archive Audit data older than (days)
SELECT	@SchemaSQL = 'IF NOT EXISTS( SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''Archive Audit data older than (days)'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''Archive Audit data older than (days)'',''30'',''User configurable - Immediate change. No action required. Audit data older than this number of days will be moved to the archive tables when the pBtAuditArchive stored procedure is executed.'');'
		EXEC (@SchemaSQL)
--Delete Audit data older than (days)
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''Delete Audit data older than (days)'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''Delete Audit data older than (days)'',''365'',''User configurable - Immediate change. No action required. Audit data older than this number of days will be deleted permanently when the pBtAuditArchive stored procedure is executed.'');'
		EXEC (@SchemaSQL)
--RowHistory View Scope
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''RowHistory View Scope'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''RowHistory View Scope'',''Active'',''User configurable - Must execute pBtAuditRebuild(All) or pBtAudit(All) to apply change. Determines source of data when _RowHistory views are created. Valid entries are: "Active", "Archive", "All".'');'
		EXEC (@SchemaSQL)
--Deleted View Scope
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''Deleted View Scope'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''Deleted View Scope'',''Active'',''User configurable - Must execute pBtAuditRebuild(All) or pBtAudit(All) to apply change. Determines source of data when _Deleted views are created. Valid entries are: "Active", "Archive", "All".'');'
		EXEC (@SchemaSQL)
...
VALUES	(''Default _Deleted view Creation Flag'',''1'',''User configurable - 0 = _Deleted view is not created, 1 = _Deleted view is created.'');'
		EXEC (@SchemaSQL)
--Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag
SELECT	@SchemaSQL = 'IF NOT EXISTS (SELECT 1 FROM ' + quotename(@AuditSchema) + '.[AuditSettings] ' 
+ 'WHERE [SettingName] = ''Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag'' )' + Char(13) + Char(10)
+ 'INSERT	' + quotename(@AuditSchema) + '.[AuditSettings] ([SettingName],[SettingValue],[AdditionalInfo])
VALUES	(''Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag'',''1'',''System setting added by BtAudit installation SQL script.  Do not change manually in the table.'');'
		EXEC (@SchemaSQL)
...
IF Object_id(quotename(@AuditSchema) + '.[AuditAllExclusions]') IS NULL
	BEGIN
		SET @SchemaSQL = 'CREATE TABLE ' + quotename(@AuditSchema) + '.[AuditAllExclusions] (
			...
			values (''' + @AuditSchema + ''',''AuditAllExclusions'',''Added by BtAudit Setup'');
			Insert ' + quotename(@AuditSchema) + '.[AuditAllExclusions] (SchemaName, TableName, Comment)
			values (''' + @AuditSchema + ''',''AuditBaseTables'',''Added by BtAudit Setup'');
			Insert ' + quotename(@AuditSchema) + '.[AuditAllExclusions] (SchemaName, TableName, Comment)
			values (''' + @AuditSchema + ''',''AuditSettings'',''Added by BtAudit Setup'');'
		EXEC (@SchemaSQL)
	END
ELSE
	BEGIN
			...
			BEGIN
				...
				Begin try
					...
					SET @SchemaSQL = 'Update ' + quotename(@AuditSchema) + '.[AuditAllExclusions] 
						set Comment = ''Added by BtAudit Setup'' 
...
Print 'Creating BtAudit table indexes'
...
-- Creating vAudit view - This view only supports single column PK's to maintain compatibility with legacy versions of BtAudit
--******************************************************************************************************************************
...
SET @SchemaSQL = 'CREATE VIEW ' + quotename(@AuditSchema) + '.[vAudit]' + Char(13) + Char(10)
       ...
       + ' -- generated by BtAudit Version ' + @Version + ' on ' + Convert(VARCHAR(30), GetDate(),100)  + Char(13) + Char(10)
       + ' -- created by Paul Nielsen and John Sigouin ' + Char(13) + Char(10)
       + ' -- www.SQLServerBible.com ' + Char(13) + Char(10)
       + ' -- BtAudit.codeplex.com ' + Char(13) + Char(10)
       + ' -- This view returns data from the live BtAudit tables formatted to match the BtAudit 2.00h Audit table.' + Char(13) + Char(10) + Char(13) + Char(10)
       ...
       + '			,AH.[operator_id]' + Char(13) + Char(10)
...
-- Upgrade BtAudit 2.00h to 3.00 if required
-- Migrate existing data to new table structure
--*********************************************************

Print ''
Print 'Searching for existing BtAudit components' 
DECLARE @SchemaSQL NVARCHAR(max)
DECLARE @AuditSchema VARCHAR(50)	
SELECT @AuditSchema = [SettingValue] from #Settings where [SettingName] = 'AuditSchema'
DECLARE @Version VARCHAR(5)	
...
IF Object_id('dbo.[LegacyAudit]') IS NOT NULL
	BEGIN --the old Audit table has been found import the data into the new structure
		Print '***** Existing BtAudit components have been found.'
		...
		--verify that all of the Audit records have been imported to BtAudit
		...
		WHILE @@Fetch_Status = 0 
		  BEGIN
				...
				--exec pBtAuditDrop @dSchemaName, @dTableName
		...
		raiserror ('***** Legacy BtAudit components have been renamed.',0,0) with nowait
		raiserror ('***** You may drop [LegacyAudit_Migrated] and [LegacySchemaAudit_Migrated] as you wish.',0,0) with nowait
		raiserror ('***** Migration of legacy audit data to new BtAudit tables completed.',0,0) with nowait
		print ''
	END --IF Object_id('dbo.[LegacyAudit]') IS NOT NULL


--**************** Setup Script Section #4 ****************
-- Drop existing BtAudit Stored Procedures
--*********************************************************

Print 'Dropping previous revision of BtAudit components' 

--drop the legacy "dbo" owned sp's
IF Object_id('pBtAudit') IS NOT NULL
  DROP PROC pBtAudit
IF Object_id('pBtAuditAll') IS NOT NULL
  DROP PROC pBtAuditAll
IF Object_id('pBtAuditRebuild') IS NOT NULL
  DROP PROC pBtAuditRebuild
IF Object_id('pBtAuditRebuildAll') IS NOT NULL
  DROP PROC pBtAuditRebuildAll
IF Object_id('pBtAuditDrop') IS NOT NULL
  DROP PROC pBtAuditDrop
IF Object_id('pBtAuditDropAll') IS NOT NULL
  DROP PROC pBtAuditDropAll
IF Object_id('pBtAuditSetTriggerState') IS NOT NULL
  DROP PROC pBtAuditSetTriggerState
IF Object_id('pBtAuditSetTriggerStateAll') IS NOT NULL
  DROP PROC pBtAuditSetTriggerStateAll

--drop the "schema" owned sp's
IF Object_id(quotename(@AuditSchema) + '.[pBtAudit]') IS NOT NULL
  BEGIN
	SET @SchemaSQL = 'DROP PROC ' + quotename(@AuditSchema) + '.[pBtAudit];'
	EXEC (@SchemaSQL)
  END
IF Object_id(quotename(@AuditSchema) + '.[pBtAuditAll]') IS NOT NULL
  BEGIN
	SET @SchemaSQL = 'DROP PROC ' + quotename(@AuditSchema) + '.[pBtAuditAll];'
	EXEC (@SchemaSQL)
  END
IF Object_id(quotename(@AuditSchema) + '.[pBtAuditDrop]') IS NOT NULL
  BEGIN
	SET @SchemaSQL = 'DROP PROC ' + quotename(@AuditSchema) + '.[pBtAuditDrop];'
	EXEC (@SchemaSQL)
  END
IF Object_id(quotename(@AuditSchema) + '.[pBtAuditDropAll]') IS NOT NULL
  BEGIN
	SET @SchemaSQL = 'DROP PROC ' + quotename(@AuditSchema) + '.[pBtAuditDropAll];'
	EXEC (@SchemaSQL)
  END
IF Object_id(quotename(@AuditSchema) + '.[pBtAuditRebuild]') IS NOT NULL
  BEGIN
	SET @SchemaSQL = 'DROP PROC ' + quotename(@AuditSchema) + '.[pBtAuditRebuild];'
	EXEC (@SchemaSQL)
  END
IF Object_id(quotename(@AuditSchema) + '.[pBtAuditRebuildAll]') IS NOT NULL
  BEGIN
	SET @SchemaSQL = 'DROP PROC ' + quotename(@AuditSchema) + '.[pBtAuditRebuildAll];'
	EXEC (@SchemaSQL)
  END

IF Object_id(quotename(@AuditSchema) + '.[pBtAuditArchive]') IS NOT NULL
  BEGIN
	SET @SchemaSQL = 'DROP PROC ' + quotename(@AuditSchema) + '.[pBtAuditArchive];'
	EXEC (@SchemaSQL)
  END

IF Object_id(quotename(@AuditSchema) + '.[pBtAuditSetTriggerState]') IS NOT NULL
  BEGIN
	SET @SchemaSQL = 'DROP PROC ' + quotename(@AuditSchema) + '.[pBtAuditSetTriggerState];'
	EXEC (@SchemaSQL)
  END

IF Object_id(quotename(@AuditSchema) + '.[pBtAuditSetTriggerStateAll]') IS NOT NULL
  BEGIN
	SET @SchemaSQL = 'DROP PROC ' + quotename(@AuditSchema) + '.[pBtAuditSetTriggerStateAll];'
	EXEC (@SchemaSQL)
  END
...
CREATE TRIGGER [BtSchemaAuditDDLTrigger]
ON DATABASE
...
IF @ContextInfo = 0x1
  ...
  If 0 = isnull((SELECT [SettingValue] from ' + quotename(@AuditSchema) + '.[AuditSettings] where [SettingName] = ''BtSchemaAuditDDLTrigger Enabled Flag''),1)
  ...
  If 1 = isnull((SELECT [SettingValue] from ' + quotename(@AuditSchema) + '.[AuditSettings] where [SettingName] = ''Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag''),1)
  IF @EventType = ''ALTER_TABLE''
    BEGIN 
      SET @SQL = ''EXEC ' + quotename(@AuditSchema) + '.pBtAuditRebuild @SchemaName = '''''' + @Schema + '''''', @TableName = '''''' + @Object + ''''''''
      EXEC (@SQL)
    END    
  IF @EventType = ''DROP_TABLE''
    BEGIN 
      SET @SQL = ''EXEC ' + quotename(@AuditSchema) + '.pBtAuditDrop @SchemaName = '''''' + @Schema + '''''', @TableName = '''''' + @Object + ''''''''
      EXEC (@SQL)
    END    
END   
'
...
SET @SchemaSQL = 'CREATE VIEW ' + quotename(@AuditSchema) + '.[vAuditArchive]' + Char(13) + Char(10)
       ...
       --+ ' -- www.SQLServerBible.com ' + Char(13) + Char(10)
       --+ ' -- BtAudit.codeplex.com ' + Char(13) + Char(10)
       + ' -- This view returns data from the Archive BtAudit tables. ' + Char(13) + Char(10) + Char(13) + Char(10)
...
SET @SchemaSQL = 'CREATE VIEW ' + quotename(@AuditSchema) + '.[vAuditAll]' + Char(13) + Char(10)
       ...
       --+ ' -- www.SQLServerBible.com ' + Char(13) + Char(10)
       --+ ' -- BtAudit.codeplex.com ' + Char(13) + Char(10)
       + ' -- This view returns data from the Active and Archive BtAudit tables. ' + Char(13) + Char(10) + Char(13) + Char(10)
...
SET @SchemaSQL = 'CREATE VIEW ' + quotename(@AuditSchema) + '.[vAuditHeaderAll]' + Char(13) + Char(10)
       ...
       --+ ' -- www.SQLServerBible.com ' + Char(13) + Char(10)
       --+ ' -- BtAudit.codeplex.com ' + Char(13) + Char(10)
       + ' -- This view returns data from the AuditHeader and AuditHeaderArchive BtAudit tables. ' + Char(13) + Char(10) + Char(13) + Char(10)
...
SET @SchemaSQL = 'CREATE VIEW ' + quotename(@AuditSchema) + '.[vAuditDetailAll]' + Char(13) + Char(10)
       ...
       --+ ' -- www.SQLServerBible.com ' + Char(13) + Char(10)
       --+ ' -- BtAudit.codeplex.com ' + Char(13) + Char(10)
       + ' -- This view returns data from the AuditDetail and AuditDetailArchive BtAudit tables. ' + Char(13) + Char(10) + Char(13) + Char(10)
...
go --------------------------------------------------------------------
Print 'Creating Stored Procedure - pBtAuditArchive'
DECLARE @SchemaSQL nVARCHAR(max)
DECLARE @AuditSchema VARCHAR(50)	
SELECT @AuditSchema = [SettingValue] from #Settings where [SettingName] = 'AuditSchema'
DECLARE @Version VARCHAR(5)	
...
SET @SchemaSQL = 'CREATE PROC ' + quotename(@AuditSchema) + '.[pBtAuditArchive]' + Char(13) + Char(10)
       + '(' + Char(13) + Char(10) 
...
Print 'Creating Stored Procedure - pBtAudit'
go
CREATE PROC pBtAudit 
	(
	@SchemaName			sysname		= 'dbo',--this is the default schema name for the tables getting BtAudit added
	@TableName			sysname,			--enter the name of the table to add BtAudit to.
...
-- Created for BtAudit Version 3.30a
-- created by Paul Nielsen and John Sigouin
-- www.SQLServerBible.com
-- BtAudit.codeplex.com
-- This SP is used to add BtAudit to a specified table.
...
--get the schema for the BtAudit objects
--*********************************************************************************
Set @AuditSchema = null		--set this manually if you have more than one instance 
							--of BtAudit objects in the database. Otherwise leave 
							--it set to null
--*********************************************************************************
If @AuditSchema is null
	Begin
		If (Select count(*) from sys.objects where name='AuditSettings' and [type] ='U') > 1
			Begin
				Raiserror ('There is more than 1 instance of BtAudit in this db. @AuditSchema MUST be set manually in the pBtAudit Stored Procedure.',16,0)
				Return
			End
		Else
...
-- script to create BtAudit triggers
...
PRINT 'Creating BtAudit for table: ' + quotename(@SchemaName) + '.' + quotename(@TableName) 
...
PRINT '	Dropping existing BtAudit components'
        ...
        ELSE   
		  ...
		  If @AddExtendedProperties = '1'
				SET @SQL = @SQL + 'EXEC sys.sp_addextendedproperty 
				  ...
				  @name = N''MS_Description'', @value = N''Column added by BtAudit'''
        EXEC (@SQL)
      END
...
SET @SQL = 'CREATE TRIGGER ' + quotename(@SchemaName) + '.' + quotename(@TableName + '_Audit_Insert') + ' ON ' + quotename(@SchemaName) + '.' + quotename(@TableName) + Char(13) + Char(10)
       ...
       --+ ' -- created by Paul Nielsen and John Sigouin ' + Char(13) + Char(10)
       --+ ' -- www.SQLServerBible.com ' + Char(13) + Char(10)
       --+ ' -- BtAudit.codeplex.com ' + Char(13) + Char(10) + Char(13) + Char(10)
       ...
       + 'declare @operator_id varchar(50)' + Char(13) + Char(10)
       + 'if object_id(N''tempdb..#@BT@DATA@Audit@UserInfo@TEMP'',N''U'') is not null' + Char(13) + Char(10)
       + '	begin' + Char(13) + Char(10)
       + '			select @operator_id=operator_id from #@BT@DATA@Audit@UserInfo@TEMP' + Char(13) + Char(10)
       + '	end' + Char(13) + Char(10)
...
SELECT @SQL = @SQL 
    ...
    + '   BEGIN' + Char(13) + Char(10)
    + '     IF @DebugFlag = 1 PRINT ''BtAudit EnabledFlag set to "false" for this table in the AuditBaseTables table. Exiting trigger...''' + Char(13) + Char(10)
	+ ' 	return' + Char(13) + Char(10)
    + '   END' + Char(13) + Char(10) + Char(13) + Char(10)
...
-- Insert the AuditHeader row
	select @SQL = @SQL
          + Char(13) + Char(10)
		      ...
		      + '			PrimaryKey, PrimaryKey2, PrimaryKey3, PrimaryKey4, PrimaryKey5, RowDescription, SecondaryRow, [RowVersion],operator_id)' + Char(13) + Char(10)
          ...
          + '        1,@operator_id' + Char(13) + Char(10)      -- the RowVersion should always be 1 initially.  The RowVersion adjustment bellow sets it to the correct value
      ...
      + 'DECLARE ' + Char(13) + Char(10)
       ...
       + '  Begin Try' + Char(13) + Char(10)
              ...
              SELECT @SQL = @SQL
                 ...
                 + '      BEGIN ' + Char(13) + Char(10)
                 + '        RAISERROR(''Update of ' + @CreatedColumnName + ', ' + @CreatedByColumnName + ', ' + @ModifiedColumnName + ', ' + @ModifiedByColumnName + ', or ' + @RowVersionColumnName + ' not permitted by BtAudit when StrictUserContext is enabled.'', 16,1)' + Char(13) + Char(10)
...
--  Insert the AuditHeader row for NO PK changes
	select @SQL = @SQL  
         + '   INSERT @PrimaryKeys select distinct PrimaryKey from @AuditDetailUpdate '  + Char(13) + Char(10)  
         + '   INSERT ' + quotename(@AuditSchema) + '.AuditHeader (AuditDate, SysUser, Application, HostName, TableName, Operation, SQLStatement, PrimaryKey, PrimaryKey2, PrimaryKey3, PrimaryKey4, PrimaryKey5, RowDescription, SecondaryRow, [RowVersion],operator_id) '  + Char(13) + Char(10)  
		 + '   OUTPUT  inserted.AuditHeaderID, quotename(inserted.PrimaryKey) + isnull(quotename(inserted.PrimaryKey2),'''') + isnull(quotename(inserted.PrimaryKey3),'''') + isnull(quotename(inserted.PrimaryKey4),'''') + isnull(quotename(inserted.PrimaryKey5),''''), inserted.[RowVersion] into @Keys (AuditHeaderID, PrimaryKey, NextRowVersion)' + Char(13) + Char(10) 
         + '     SELECT '
       ...
       + '        NULL,     -- Secondary Row Value (e.g. Order Number for an Order Detail Line)' + Char(13) + Char(10)
          + case @BaseTableDDL 
				when 1 then ' deleted.' + @RowVersionColumnName + ' + 1,@operator_id' + Char(13) + Char(10)
				ELSE 		'1,@operator_id' + Char(13) + Char(10)
			END
       + '          FROM  inserted' + Char(13) + Char(10)
       + '             JOIN deleted' + Char(13) + Char(10)
       + '               ON ' + replace(@PKColumnNameConcatenationWithQuotenameAndDateConvert,'src.[','inserted.[') + ' = ' + replace(@PKColumnNameConcatenationWithQuotenameAndDateConvert,'src.[','deleted.[') + Char(13) + Char(10)       
...
SET @SQL = @SQL + '			)' + Char(13) + Char(10) + Char(13) + Char(10) 
       + ' -- NEW generated by BtAudit Version ' + @Version + ' on ' + Convert(VARCHAR(30), GetDate(),100)  + Char(13) + Char(10)
       --+ ' -- created by Paul Nielsen and John Sigouin ' + Char(13) + Char(10)
       --+ ' -- www.SQLServerBible.com ' + Char(13) + Char(10)
       --+ ' -- BtAudit.codeplex.com ' + Char(13) + Char(10)
...
--save base table BtAudit settings to AuditBaseTables table
...
Print 'Creating Stored Procedure - pBtAuditRebuild'
go
CREATE PROC pBtAuditRebuild (
...
-- www.SQLServerBible.com
-- BtAudit.codeplex.com
-- This SP is used to rebuild BtAudit triggers for the specified table based on the settings saved in the AuditBaseTables table.
...
--retrieve base table BtAudit settings from AuditBaseTables table
...
IF @@rowcount = 0
	BEGIN
		--RAISERROR ('INFO - BtAudit cannot be rebuilt for table [%s].[%s] because it''s settings don''t exist in the AuditBaseTables table.',0,0,@SchemaName,@TableName);
		RETURN
	END
ELSE
	PRINT 'Rebuilding BtAudit for table: ' + quotename(@SchemaName) + '.' + quotename(@TableName) 
	SET @SQL = 'EXEC ' + quotename(@AuditSchema) + '.pBtAudit' + Char(13) + Char(10)
...
Print 'Creating Stored Procedure - pBtAuditRebuildAll'
go

CREATE PROC [pBtAuditRebuildAll] 
...
-- www.SQLServerBible.com
-- BtAudit.codeplex.com
-- This SP is used to rebuild BtAudit triggers for ALL tables that are currently setup with BtAudit based on the settings saved in the AuditBaseTables table.
...
WHILE @@Fetch_Status = 0 
  BEGIN
		SET @SQL = 'EXEC ' + quotename(@AuditSchema) + '.pBtAuditRebuild ''' + @SchemaName + ''', ''' + @TableName + ''''
...
Print 'Creating Stored Procedure - pBtAuditDrop'
go

CREATE PROC pBtAuditDrop (
...
-- www.SQLServerBible.com
-- BtAudit.codeplex.com
-- This SP is used to remove BtAudit triggers for the specified table.
...
Print 'Dropping BtAudit components from table: ' + quotename(@SchemaName) + '.' + quotename(@TableName)
IF @DropBaseTableDDLColumns = 1
	BEGIN
		set context_info 0x1;
...
Print 'Creating Stored Procedure - pBtAuditAll'
go
...
-- www.SQLServerBible.com
-- BtAudit.codeplex.com
-- This SP is used to create BtAudit triggers for all tables in the current database 
-- except for system tables, BtAudit tables and the tables listed in the AuditAllExclusions table.
...
WHILE @@Fetch_Status = 0 
  BEGIN
		SET @SQL = 'EXEC ' + quotename(@AuditSchema) + '.pBtAudit ''' + @SchemaName + ''', ''' + @TableName + ''''
...
Print 'Creating Stored Procedure - pBtAuditSetTriggerState'
go
Create PROC [pBtAuditSetTriggerState] 
	(
	@SchemaName sysname = 'dbo',  --this is the default schema name for the tables getting BtAudit added
	@TableName sysname,
...
-- www.SQLServerBible.com
-- BtAudit.codeplex.com
-- This SP is used to enable/disable BtAudit triggers at the SQL Server level for the specified table.

SET NoCount ON
 
DECLARE @SQL NVARCHAR(max),
		@AuditSchema VARCHAR(50),
		@AutopBtAuditRebuildCurrentSetting varchar(100)

--get @AuditSchema 
SELECT @AuditSchema = [SettingValue] from [AuditSettings] where [SettingName] = 'AuditSchema'

--get [AuditSettings] - Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag
Select	@SQL = 'SELECT @SettingValue = [SettingValue] from ' + quotename(@AuditSchema) + '.[AuditSettings] where [SettingName] = ''Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag'''
EXEC sp_executesql @SQL, N'@SettingValue varchar(100) OUTPUT', @AutopBtAuditRebuildCurrentSetting OUTPUT

--disable auto trigger rebuild on schema changes
--set [AuditSettings] = 0 - Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag
If @AutopBtAuditRebuildCurrentSetting = 1
	BEGIN
		Select	@SQL = 'Update ' + quotename(@AuditSchema) + '.[AuditSettings] Set [SettingValue] = 0 where [SettingName] = ''Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag'''
		EXEC (@SQL)
	END
...
--set [AuditSettings] = 1 - Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag
If @AutopBtAuditRebuildCurrentSetting = 1
	BEGIN
		Select	@SQL = 'Update ' + quotename(@AuditSchema) + '.[AuditSettings] Set [SettingValue] = 1 where [SettingName] = ''Launch pBtAuditRebuild from BtSchemaAuditDDLTrigger Enabled Flag'''
print (@SQL)
		EXEC (@SQL)
	END
...
Print 'Creating Stored Procedure - pBtAuditSetTriggerStateAll'
go

CREATE PROC [pBtAuditSetTriggerStateAll] 
	(
	@InsertEnabledFlag BIT = 1,	-- State for Insert Trigger: 1 = enabled, 0 = disabled
	@UpdateEnabledFlag BIT = 1,	-- State for Update Trigger: 1 = enabled, 0 = disabled
	@DeleteEnabledFlag BIT = 1	-- State for Delete Trigger: 1 = enabled, 0 = disabled
...
-- www.SQLServerBible.com
-- BtAudit.codeplex.com
-- This SP is used to enable/disable BtAudit triggers at the SQL Server level for all tables that are setup with BtAudit.
...
WHILE @@Fetch_Status = 0 
  BEGIN
		SET @SQL = 'EXEC ' + quotename(@AuditSchema) + '.pBtAuditSetTriggerState ''' + @SchemaName + ''', ''' + @TableName + ''', '
...
Print 'Creating Stored Procedure - pBtAuditDropAll'
go
CREATE PROC [pBtAuditDropAll] 
   (
...
-- www.SQLServerBible.com
-- BtAudit.codeplex.com
-- This SP is used to remove BtAudit triggers for the all tables that are setup with BtAudit.
...
If	@DropAuditTables = 0 and 
	...
	@DropBaseTableViews = 0
		Begin
			Raiserror ('You MUST set the input parameters to identify which components of BtAudit you want to drop.
	@DropAuditTables bit=0,				0 = don''t drop audit tables, 1 = drop audit tables
										    if @DropAuditTables=1 then @DropAuditViews,@DropBaseTableTriggers,
...
If LOWER(isnull(@ConfirmAllDrop, 'no')) <> 'yes'
	begin
			Raiserror ('Executing this SP completely removes some or all BtAudit components from this database including the BtAudit added DDL columns from the base tables.
...
IF @DropAuditDDLTriggers = 1
	BEGIN
		Print 'Dropping BtSchemaAuditDDLTrigger'
		IF Exists(select * from sys.triggers where name = 'BtSchemaAuditDDLTrigger')
		  DROP TRIGGER BtSchemaAuditDDLTrigger ON Database
	END
...
IF @DropAuditSPs = 1
	BEGIN
		...
		-- remove pBtAudit SP
		SET @SQL = 'IF Object_id(''' + quotename(@AuditSchema) + '.pBtAudit'') IS NOT NULL
		  DROP PROC ' + quotename(@AuditSchema) + '.pBtAudit'
		EXEC(@SQL)

		-- remove pBtAuditAll SP
		SET @SQL = 'IF Object_id(''' + quotename(@AuditSchema) + '.pBtAuditAll'') IS NOT NULL
		  DROP PROC ' + quotename(@AuditSchema) + '.pBtAuditAll'
		EXEC(@SQL)

		-- remove pBtAuditRebuild SP
		SET @SQL = 'IF Object_id(''' + quotename(@AuditSchema) + '.pBtAuditRebuild'') IS NOT NULL
		  DROP PROC ' + quotename(@AuditSchema) + '.pBtAuditRebuild'
		EXEC(@SQL)

		-- remove pBtAuditRebuildAll SP
		SET @SQL = 'IF Object_id(''' + quotename(@AuditSchema) + '.pBtAuditRebuildAll'') IS NOT NULL
		  DROP PROC ' + quotename(@AuditSchema) + '.pBtAuditRebuildAll'
		EXEC(@SQL)

		-- remove pBtAuditSetTriggerState SP
		SET @SQL = 'IF Object_id(''' + quotename(@AuditSchema) + '.pBtAuditSetTriggerState'') IS NOT NULL
		  DROP PROC ' + quotename(@AuditSchema) + '.pBtAuditSetTriggerState'
		EXEC(@SQL)

		-- remove pBtAuditSetTriggerStateAll SP
		SET @SQL = 'IF Object_id(''' + quotename(@AuditSchema) + '.pBtAuditSetTriggerStateAll'') IS NOT NULL
		  DROP PROC ' + quotename(@AuditSchema) + '.pBtAuditSetTriggerStateAll'
		EXEC(@SQL)

		-- remove pBtAuditArchive SP
		SET @SQL = 'IF Object_id(''' + quotename(@AuditSchema) + '.pBtAuditArchive'') IS NOT NULL
		  DROP PROC ' + quotename(@AuditSchema) + '.pBtAuditArchive'
		EXEC(@SQL)
	END
...
WHILE @@Fetch_Status = 0 
  BEGIN
		SET @SQL = 'EXEC ' + quotename(@AuditSchema) + '.pBtAuditDrop @SchemaName=''' + @SchemaName + '''' +
					', @TableName=''' + @TableName + '''' +
...
-- remove pBtAuditDrop SP
IF @DropAuditSPs = 1
	BEGIN
		SET @SQL = 'IF Object_id(''' + quotename(@AuditSchema) + '.pBtAuditDrop'') IS NOT NULL
		  DROP PROC ' + quotename(@AuditSchema) + '.pBtAuditDrop'
		EXEC(@SQL)
	END

-- remove pBtAuditDropAll SP
IF @DropAuditSPs = 1
	BEGIN
		SET @SQL = 'IF Object_id(''' + quotename(@AuditSchema) + '.pBtAuditDropAll'') IS NOT NULL
		  DROP PROC ' + quotename(@AuditSchema) + '.pBtAuditDropAll'
		EXEC(@SQL)
	END
go
...
SET @SQL = 'ALTER SCHEMA ' + quotename(@AuditSchema) + ' TRANSFER dbo.pBtAudit'
EXEC (@SQL)
SET @SQL = 'ALTER SCHEMA ' + quotename(@AuditSchema) + ' TRANSFER dbo.pBtAuditAll'
EXEC (@SQL)
SET @SQL = 'ALTER SCHEMA ' + quotename(@AuditSchema) + ' TRANSFER dbo.pBtAuditDrop'
EXEC (@SQL)
SET @SQL = 'ALTER SCHEMA ' + quotename(@AuditSchema) + ' TRANSFER dbo.pBtAuditDropAll'
EXEC (@SQL)
SET @SQL = 'ALTER SCHEMA ' + quotename(@AuditSchema) + ' TRANSFER dbo.pBtAuditRebuild'
EXEC (@SQL)
SET @SQL = 'ALTER SCHEMA ' + quotename(@AuditSchema) + ' TRANSFER dbo.pBtAuditRebuildAll'
EXEC (@SQL)
SET @SQL = 'ALTER SCHEMA ' + quotename(@AuditSchema) + ' TRANSFER dbo.pBtAuditSetTriggerState'
EXEC (@SQL)
SET @SQL = 'ALTER SCHEMA ' + quotename(@AuditSchema) + ' TRANSFER dbo.pBtAuditSetTriggerStateAll'
EXEC (@SQL)
...
-- rebuild all existing BtAudit Triggers
--**********************************************************
...
If @RebuildTriggersAfterInstall = 1
	begin
		SET @SQL = quotename(@AuditSchema) + '.pBtAuditRebuildAll'
		EXEC (@SQL)
	end
Else
	begin
		Print '*** Execute pBtAuditRebuild or pBtAuditRebuildAll to upgrade BtAudit triggers and views. ***'
	end
